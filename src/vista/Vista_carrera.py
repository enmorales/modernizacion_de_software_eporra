...
class Vista_carrera(QWidget):
    ...
    def guardar_cambios(self):
        ...
        """ 
        carrera_guardada = self.interfaz.guardar_carrera(self.texto_nombre.text(),self.competidores)
        if not(carrera_guardada):
            self.mostrar_mensaje('Error', 
                                 ('Surgió un error al guardar la Carrera por uno de los siguientes motivos: \n'
                                  'El nombre de la carrera es vacío, mayor a 200 caracteres o ya existe en EPorra \n'
                                  'Algún nombre de competidores es vacío, mayor a 200 caracteres o se repite \n'
                                  'Alguna probabilidad no está en el rango de 0 a 1 \n'
                                  'La suma de probabilidades es diferente a 1 \n'))
        else: 
